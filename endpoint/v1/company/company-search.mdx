...
<Tip>
  **Smart Workflow**: Use Company Search to discover companies, then check the
  `updateDate`. If the data is recent enough (e.g., within last 30 days), use it
  directly. If not, fetch fresh data using Company Profile Data for critical
  targets.
</Tip>
...
<Note>
  The `updateDate` field indicates when the company was last refreshed in our
...
3. **Enrich** - Use [Company Profile Data](/endpoint/v1/company/extract-profile) only for companies that need even fresher data
